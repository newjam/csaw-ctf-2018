from sympy import Eq, solveset, symbols, S
...
def solve(equation):
  #x = list(equation.free_symbols)[0]
  x = symbols('X')
...
def foo():
  ...
  while True:
    ...
    if line == b'YAAAAAY keep going':
      pass
    elif line == b'Sorry Try Again ):':
      raise Exception(line)
    elif line == b'HEYYYY THAT IS NOT VALID INPUT REMEMBER WE ONLY ACCEPT DECIMALS!':
      raise Exception(line)
...
def test():
  blah = '((((1 * X) * (14 - 14)) + ((14 + 9) + (14 * 3))) * (((10 * 6) * (17 + 14)) - ((6 - 12) * (10 * 18)))) + ((((7 + 11) + (20 + 2)) * ((2 - 18) * (14 * 5))) + (((9 + 20) - (18 * 3)) - ((2 - 20) * (1 + 13)))) = 146527'
  eq = parse(blah)
  sol = solveset(eq)
  print(sol)
  return sol
